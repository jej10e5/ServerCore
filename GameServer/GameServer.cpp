...
int main()
{
	...
	// 단점) FD_SETSIZE(64)개만 사용 가능
	

	// socket set
	// 1) 읽기[ ] 쓰기[ ] 예외(OOB)[ ] 관찰 대상 등록
	// OutOfBand는 send(I) 마지막 인자 MSG_OOB로 보내는 특별한 데이터
	// 반든 쪽에서도 recv OOB 세팅을 해야 읽을 수 있음
	...
	while (true)
	{
		...
		int32 retVal = ::select(0, &reads, &writes, nullptr, nullptr); // 무한대기
